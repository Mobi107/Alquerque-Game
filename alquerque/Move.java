...
class Move {
    ...
    /** Return a move from linearized index K1 to K2. */
    static Move move(int k1, int k2) {
        return move(col(k1), row(k1), col(k2), row(k2), null);
    }
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.jumpTail() == null) {
            ...
            return move(move0.jumpTail(), move1);
        }
     ...
     *  the left. */
    boolean isLeftMove() {
        return col1() - col0() == -1 && row1() == row0() && _nextJump == null;
    }
    ...
    /** Return true iff this is a vertical, non-capturing move
     * for player P. */
    boolean isVerticalMove(PieceColor p) {
        if (p == PieceColor.BLACK) {
            return row1() - row0() == -1 && col1() == col0()
                    && _nextJump == null;
        } else {
            return row1() - row0() == 1 && col1() == col0()
                    && _nextJump == null;
     ...
     * for player P. */
    boolean isDiagonalMove(PieceColor p) {
        if (_fromIndex % 2 == 0) {
            if (p == PieceColor.BLACK) {
                return row1() - row0() == -1
                        && abs(col1() - col0()) == 1 && _nextJump == null;
            } else {
                return row1() - row0() == 1
                        && abs(col1() - col0()) == 1 && _nextJump == null;
            }
    ...
    /** Returns true iff this is a single move for player P. */
    boolean isSingleMove(PieceColor p) {
        return isLeftMove() || isRightMove() || isVerticalMove(p)
                || isDiagonalMove(p);
    }

    /** Returns true iff this is a horizontal jump from index K to index I. */
    boolean isHorizontalJump(int k, int i) {
        return abs(col(k) - col(i)) == 2 && row(k) == row(i);
    }

    /** Returns true iff this is a vertical jump from index K to index I. */
    boolean isVerticalJump(int k, int i) {
        return abs(row(k) - row(i)) == 2 && col(k) == col(i);
    }

    /** Returns true iff this is a diagonal jump from index K to index I. */
    boolean isDiagonalJump(int k, int i) {
        return abs(row(k) - row(i)) == 2 && abs(col(k) - col(i)) == 2
                && k % 2 == 0;
    }

    /** Returns true iff this is a valid jump from index K to index I. */
    boolean isValidJump(int k, int i) {
        return isHorizontalJump(k, i) || isVerticalJump(k, i)
                || isDiagonalJump(k, i);
    }
    ...
    private void toString(Formatter out) {
        if (jumpTail() != null) {
            out.format("%s%s-%s", col0(), row0(), jumpTail().toString());
        } else {
